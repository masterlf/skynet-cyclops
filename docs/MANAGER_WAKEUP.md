...
record maps to `supported,false`; a current enabled and matching record maps to `supported,true`;
every unavailable, malformed, unverifiable, wrong-state, or drifted case maps to
`unsupported,false`. Private paths, identifiers, hashes, versions and reason codes never enter
projection. The validator runs after task-scope denial but before lifecycle reconciliation, lease
acquisition, budget consumption, or any model construction.
